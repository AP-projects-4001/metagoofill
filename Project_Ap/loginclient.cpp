...
bool checkingUserAndPass(string username,string password)
{
    ...
    {
        char_array_to_string(user, 16, Client.get_User());
        char_array_to_string(pass, 31, Client.get_Password());
        if(Client.get_access() == '1')
        {
            if(username == user)
            {
                if(password == pass)
...
void LoginClient::on_pushButton_clicked()
{
    ...
    if(checkingUserAndPass(User, Pass) && User != "" && Pass != "")
    {
        ...
        while(cus_tmps.read((char*)&cli_tmp, sizeof(client)))
        {
