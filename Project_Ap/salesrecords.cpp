...
void SalesRecords::search_in_sells(string &sells)
{
    ...
    for(int i=0;i<cust.get_number_mysells();i++){
        ...
        database_clients.seekg((factor.ID_client-1)*sizeof(client));
        ...
        sells+=to_string(i+1)+"\t"+product_name+"\t"+to_string(factor.number*factor.price)+"\t"+to_string(factor.number)+"\t"+client_user+"\n";
    }
